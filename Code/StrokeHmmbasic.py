...
global lengthBorder; lengthBorder = 300
...
class HMM:
    ...
    def __init__(self, states, features, contOrDisc, numVals):
        ...
        # states looks like: ['drawing', 'text']
        # featureNames looks like ['length']
        # featuresCorD looks like {'length': 1}
        # numVals looks like {'length': 2}
        self.states = states 
        ...
        # priors looks like {'text': 0.5272727272727272, 'drawing': 0.4727272727272727}
        # emissions looks like {'text': {'length': [0.6366083445491252, 0.3633916554508748]},
        #                       'drawing': {'length': [0.3166144200626959, 0.6833855799373041]}}
        # transitions looks like {'text': {'text': 0.8217821782178217, 'drawing': 0.1782178217821782},
        #                         'drawing': {'text': 0.10454908220271349, 'drawing': 0.8954509177972865}}
        self.priors = None
    ...
    def label( self, data):
        ...
        # data looks like [{'length': 1}, {'length': 1}, {'length': 0}, {'length': 0}, ...]
        V=[{}]
        path={}
...
class StrokeLabeler:
    ...
    def confusion(self,trueLabels, classifications):
        # In the strokeLabeler class, write a function called confusion(trueLabels, classifications) that takes
        # a list of truth labels and a list of labels output by the classifier for the same set of strokes (in
        # order).  It should return a dictionary with the following structure (based on the table above):
        ...
        for label in range(len(trueLabels)):
            ...
            if trueLabels[label]=='drawing':
                # if drawing classified as text
                if classifications[label]=='text':
                    falsePos+=1
                # if classified correctly
                elif classifications[label]=='drawing':
                    trueNeg+=1
            elif trueLabels[label]=='text':
                # if classified correctly
                if classifications[label]=='text':
                    truePos+=1
                # if text classified as drawing
                elif classifications[label]=='drawing':
                    falseNeg+=1
            counter+=1
        return {'drawing':{'drawing':trueNeg,'text':falsePos},'text':{'drawing':falseNeg,'text':truePos}}
